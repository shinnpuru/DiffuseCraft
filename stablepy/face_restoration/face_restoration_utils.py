    ...
    try:
        ...
        for cropped_face in face_helper.cropped_faces:
            ...
            try:
                ...
                logger.error('Failed face-restoration inference', exc_info=True)
...
class CommonFaceRestoration(FaceRestoration):
    ...
    def __init__(self, device):
        ...
        self.device = device
        self.net = None
        self.model_path = os.path.join(os.path.expanduser("~"), ".cache", "face_restoration_models")
    ...
    def send_model_to(self, device):
        if self.net:
            logger.debug("Sending %s to %s", self.net.__class__.__name__, device)
            self.net.to(device)
        if self.face_helper:
